...
def biogeochem_balance(n, s, L, T, I, v, k_v, RAI, root_d, Zr, r_het, r_aut, D, temp_soil, pH_in, conc_in, f_CEC_in, K_CEC, CEC_tot, Si_in, CaCO3_in, MgCO3_in, M_rock_in, t_app, mineral, rock_f_in, d_in, psd_perc_in, SSA_in, diss_f, dt, conv_Al, conv_mol, keyword_add, keyword_Si_precip,k_prec_Si,d_prev=None, psd_prev=None, rock_f_prev=None, 
                       M_rock_prev=None, M_min_prev=None,diss_f_n=0.0, t_elapsed_start=0.0):
    ...
    if M_rock_in > 0 or M_rock_prev is not None: 
        if M_rock_prev is not None:
            M_rock_in_actual = M_rock_prev
        else:
            M_rock_in_actual = M_rock_in
...
#------------------------------------------------------------------------------
    ...
    for i in range(0, len(s)):
        ...
        H_rain[i] = fsolve(equations, float(10**-6*conv_mol)) # [mol/l]
...
#------------------------------------------------------------------------------            
    ...
    # diss_f(t) = diss_f0 * (t_ref / (t + t_ref))^n
    # t=0: df = diss_f0
    # t>0: monotonically decreasing
    # t_ref = 1d (normalisation constant, controls decay rate)
    # diss_f_n = n, calibrated against May 2023 + May 2024 BaCl2 data
    t_ref = 1.0
    if diss_f_n != 0.0:
        t_steps = t_elapsed_start + np.arange(len(s)) * dt
        diss_f_arr = diss_f * (t_ref / (t_steps + t_ref)) ** diss_f_n
    else:
        diss_f_arr = np.full(len(s), diss_f)
    ...
    #Silicate weathering
    if M_rock_in > 0 or M_rock_prev is not None:
        tt_app = int(t_app/dt)  #application timestep
        
        if M_rock_prev is None:
            ...
            if t_app == 0:
                for j in range(0, number_min):
                    ...
                    Wr[j,0] = s[0]*diss_f_arr[0]*(k_diss_H_t[j,0]*(H[0]/conv_mol)**n_H[j]+k_diss_w_t[j,0]+k_diss_OH_t[j,0]*(H[0]/conv_mol)**n_OH[j])*(1-Omega[j,0]) # [mol-conv/ m2 d]
                    EW[j,0] = Wr[j,0]*SA[0]*rock_f[j,0] # [mol-conv/d]
        
        else:
            tt_app = 0
            M_rock[0]   = M_rock_prev
            rock_f[:,0] = rock_f_prev
            M_min[:,0]  = M_min_prev
            M_iner      = M_rock_prev*(1-np.sum(rock_f_prev))
            d[:,0]      = d_prev
            delta_d[:,0] = np.insert(np.diff(d[:,0]),0,d[0,0])
            psd[:,0]    = psd_prev
            lamb[:,0]   = a*d[:,0]**b
            SSA[:,0]    = 6/(d[:,0]*rho_rock)*lamb[:,0]
            SA[0]       = np.sum(SSA[:,0]*psd[:,0]*delta_d[:,0])
            for j in range(0, number_min):
                Omega[j,0] = pyEW.Omega_sil(mineral[j], Ca[0], Mg[0], K[0], Na[0], Si[0], H[0], K_sp[j], conv_mol)
                Wr[j,0] = s[0]*diss_f_arr[0]*(k_diss_H_t[j,0]*(H[0]/conv_mol)**n_H[j]+k_diss_w_t[j,0]+k_diss_OH_t[j,0]*(H[0]/conv_mol)**n_OH[j])*(1-Omega[j,0])
...
#------------------------------------------------------------------------------
    ...
    for i in range(1, len(s)): 
            ...
            #Silicate weathering
            if M_rock_in > 0 or M_rock_prev is not None:
                if i == tt_app and M_rock_prev is None:
                
                #application time
                    for j in range(0, number_min):
                        Omega[j,i] = pyEW.Omega_sil(mineral[j], Ca[i], Mg[i], K[i], Na[i], Si[i], H[i], K_sp[j], conv_mol) #[-]
                        Wr[j,i] = s[i]*diss_f_arr[i]*(k_diss_H_t[j,i]*(H[i]/conv_mol)**n_H[j]+k_diss_w_t[j,i]+k_diss_OH_t[j,i]*(H[i]/conv_mol)**n_OH[j])*(1-Omega[j,i]) # [mol-conv/ m2 d]
                ...
                #post application
                    ...
                    for j in range(0, number_min):
                        if M_min[j,i-1] != 0: 
                            Omega[j,i] = pyEW.Omega_sil(mineral[j], Ca[i], Mg[i], K[i], Na[i], Si[i], H[i], K_sp[j], conv_mol) #[-]
                            Wr[j,i] = s[i]*diss_f_arr[i]*(k_diss_H_t[j,i]*(H[i]/conv_mol)**n_H[j]+k_diss_w_t[j,i]+k_diss_OH_t[j,i]*(H[i]/conv_mol)**n_OH[j])*(1-Omega[j,i]) # [mol/ m2 d]
