...
def moisture_balance(rain, Zr, soil, ET0, v, k_v, keyword_wb, s_in,t_end,dt):
    ...
    if keyword_wb == 1:
        ...
        for i in range(0, len(rain)-1):
            ...
            if s[i]<=s_h:
                ...
                E[i] = (s[i]-s_h)/(s_i-s_h)*E0[i]*(1-v[i]/k_v[i])
            elif s[i]<=1:
                E[i] = E0[i]*(1-v[i]/k_v[i])

            # Transpiration [m/d]
            if s[i]<=s_w:
                T[i] = 0
            elif s[i]<=s_i:
                T[i] = (s[i]-s_w)/(s_i-s_w)*ET0[i]*v[i]/k_v[i]
            elif s[i]<=1:
                T[i] = ET0[i]*v[i]/k_v[i]
    ...
    if keyword_wb == 0:
        ...
        # Evaporation [m/d]
        if s_in>=s_h:
            E = (s_in-s_h)/((s_i+1)/2-s_h)*ET0*(1-v/k_v[i])
        # Transpiration [m/d]
        if s_in>=s_w and s_in<=s_i:
            T = (s-s_w)/(s_i-s_w)*ET0*v/k_v[i]
        elif s_in>=s_i:
            T = ET0*v/k_v[i]
