...
df_Clay = df_drymass[df_drymass['Treatment'].str.contains('Clay', case=False, na=False)]
df_plants = df_Clay.groupby(['Treatment', 'Plant nr'])['Dry weight biomass (g)'].sum().reset_index()
...
# ==========================================
win_size_rain = int(12/dt) # 6-hour moving average for rain
rain_smooth = pyEW.mov_avg(rain, win_size_rain)
for index, row in df_plants.iterrows():
    [s, s_w, s_i, I, L, T, E, Q, Irr, n] = pyEW.moisture_balance(
        ...
        t_end= t_end,
        dt   = dt)
    result[row['Treatment']].update({'s': s, 's_w': s_w, 's_i': s_i, 'I': I, 'L': rain_smooth, 'T': T, 'E': E, 'Q': Q, 'Irr': Irr,'n': n})
...
SOC_in = rho_bulk*13.9/1000 #[gOC/m3] initial Clay OC content
...
for index, row in df_plants.iterrows():
    ...
    [SOC_total, r_het, r_aut, D] = pyEW.respiration(
        ...
        k_dec_factor= 0.25) # decay factor for biochar, need to be calibrated
...
for ion in ions:
    ...
    if need_break:
        ...
        # 设置y轴范围
        bottom_max = p75 * 1.15  # 从75百分位往上留15%
        max_val = max(all_values)
        upper_values = [v for v in all_values if v > bottom_max]
        if upper_values:
            top_min = min(upper_values) * 0.75  # 从最小的大值往下留10%
            top_max = max(upper_values) * 1.25  # 最大值往上留10%
        else:
            top_min = p95
            top_max = max_val * 1.1
...
# Control - Clay
Mg_CC = data_CC['Mg_tot'][-1] / conv_mol + np.trapezoid(data_CC['UP_Mg'], t) / conv_mol
Ca_CC = data_CC['Ca_tot'][-1] / conv_mol + np.trapezoid(data_CC['UP_Ca'], t) / conv_mol

# Control - Biochar - Clay
data_CBC_control = data_CBC
Mg_CBC = data_CBC_control['Mg_tot'][-1] / conv_mol + np.trapezoid(data_CBC_control['UP_Mg'], t) / conv_mol
Ca_CBC = data_CBC_control['Ca_tot'][-1] / conv_mol + np.trapezoid(data_CBC_control['UP_Ca'], t) / conv_mol
...
for i in Group:
    if 'Olivine' in i:
        ...
        data_current = data_OBC if 'Biochar' in i else data_OC
        ...
        data_current = data_WBC if 'Biochar' in i else data_WC
...
# Method 2
CO2_OBS_M2 = (data_OBC['M_rock'][0]-data_OBC['M_rock'][-1])*1.25/(rho_bulk*1e-3*Zr)
CO2_OS_M2 = (data_OC['M_rock'][0]-data_OC['M_rock'][-1])*1.25/(rho_bulk*1e-3*Zr) 
CO2_WBS_M2 = (data_WBC['M_rock'][0]-data_WBC['M_rock'][-1])*0.76/(rho_bulk*1e-3*Zr)
CO2_WS_M2 = (data_WC['M_rock'][0]-data_WC['M_rock'][-1])*0.76/(rho_bulk*1e-3*Zr)
CO2_CBC_M2 = 0.0  
CO2_CC_M2 = 0.0  

#Method 3
CO2_OBS_M3 = IC_method(data_OBC['IC_tot'],data_OBC['DIC'],data_OBC['CaCO3'],data_OBC['MgCO3'])
CO2_OS_M3 = IC_method(data_OC['IC_tot'],data_OC['DIC'],data_OC['CaCO3'],data_OC['MgCO3'])
CO2_WBS_M3 = IC_method(data_WBC['IC_tot'],data_WBC['DIC'],data_WBC['CaCO3'],data_WBC['MgCO3']) 
CO2_WS_M3 = IC_method(data_WC['IC_tot'],data_WC['DIC'],data_WC['CaCO3'],data_WC['MgCO3']) 
CO2_CBC_M3 = 0.0
CO2_CC_M3 = 0.0
...
Method2_total = [CO2_CBC_M2, CO2_CC_M2, CO2_OBS_M2, CO2_OS_M2, CO2_WBS_M2, CO2_WS_M2]
Method3_total = [CO2_CBC_M3, CO2_CC_M3, CO2_OBS_M3, CO2_OS_M3, CO2_WBS_M3, CO2_WS_M3]

#Method 1 figure
fig_CDR, ax = plt.subplots(figsize=(10, 6))
x = np.arange(len(Group))
width = 0.6
ax.bar(x - width/3, Method1_CDR, width/3, label='CDR (Method 1)', color='#4C72B0')
ax.bar(x, Method2_total, width/3, label='CDR (Method 2)', color="#0FB9A2")
ax.bar(x + width/3, Method3_total, width/3, label='CDR (Method 3)', color="#FF6F59")
ax.set_ylabel('CDR (g CO$_2$ kg$^{-1}$ soil)', fontsize=12) 
ax.set_title('Comparison of CDR by Method 1, 2, and 3', fontsize=14) 
ax.set_xticks(x) 
