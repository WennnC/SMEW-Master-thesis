...
import numpy as np
from . import pyEW  # or from . import 
from scipy import optimize
from scipy.optimize import fsolve, brentq
#minimize, least_squares, newton_krylov, broyden1, root, broyden2

def biogeochem_balance(n, s, L, T, I, v, k_v, RAI, root_d, Zr, r_het, r_aut, D, temp_soil, pH_in, conc_in, f_CEC_in, K_CEC, CEC_tot, Si_in, CaCO3_in, MgCO3_in, M_rock_in, t_app, mineral, rock_f_in, d_in, psd_perc_in, SSA_in, diss_f, dt, conv_Al, conv_mol, keyword_add, keyword_Si_precip,k_prec_Si,rho_bulk, Ni_react_init, OM_pct):
    ...
    SiO2_gel = np.zeros(len(s))   # accumulative SiO2  (mol-conv)
    P_Si = np.zeros(len(s))       # precipitation rate (mol-conv/d)
    ...
    Ni_tot = np.zeros(len(s))   # [mol] total Ni (solution + adsorbed)
    Ni = np.zeros(len(s))       # [mol-conv/L] Ni_aq (aqueous phase only)

    # ── Ni solid-solution partitioning: PTF from Groenenberg et al. (2010) ─
    # Transfer functions for solid-solution partitioning of Cd, Cu, Ni, Pb, Zn
    # European Journal of Soil Science, 61(1), 58-73. doi:10.1111/j.1365-2389.2009.01201.x
    # Table 3.1, Nickel CIII (INT + Qme + OM + pH, n=1466, R²=64.1%)
    # log[Ni_dis, mol/L] = 3.40 + 0.93*log[Ni_react, mol/kg]
    #                           - 0.53*log(OM%) - 0.45*pH
    PTF_INT_Ni = 3.40
    PTF_Qme_Ni = 0.93    # reactive Ni solid-phase exponent
    PTF_OM_Ni  = -0.53   # organic matter coefficient
    PTF_pH_Ni  = -0.45   # pH coefficient
    rho_b  = rho_bulk / 1e6 * 1e3   # [kg/m³]: measured bulk density (g/m³ → kg/m³)
    M_soil = rho_b * Zr              # [kg_soil/m²]
 
    def Ni_dis_PTF(Ni_react_mol_kg, OM_pct, H_val):
        """Dissolved Ni [micromol/L] from PTF (Groenenberg et al. 2010, CIII).
        Ni_react_mol_kg: reactive Ni [mol/kg] from HNO3 extraction.
        OM_pct:          soil organic matter [%].
        H_val:           H+ concentration [mol-conv/L] (model internal units).
        Returns Ni_dis in micromol/L (mol-conv/L).
        """
        pH_val = -np.log10(max(H_val, 1e-30) / conv_mol)
        Ni_react_safe = max(Ni_react_mol_kg, 1e-15)
        OM_safe       = max(OM_pct, 0.01)
        log_Ni_dis = (PTF_INT_Ni
                      + PTF_Qme_Ni * np.log10(Ni_react_safe)
                      + PTF_OM_Ni  * np.log10(OM_safe)
                      + PTF_pH_Ni  * pH_val)
        return (10**log_Ni_dis) * conv_mol   # mol/L → micromol/L
...
#------------------------------------------------------------------------------
    ...
    [MM_Mg, MM_Ca, MM_Na, MM_K, MM_Si, MM_C, MM_Anions, MM_Al, MM_Ni]=pyEW.MM(conv_mol) 
    ...
    # SiO2 amorphous solubility (temperature-dependent)
    K_sp_SiO2_am = pyEW.K_sp_SiO2_am(T_K, conv_mol)  # [mol-conv/L], array over time
...
#------------------------------------------------------------------------------            
    ...
    if keyword_add == 1:
        ...
        I_Ni = np.mean(T+L)*1000*Ni[0]*s[0]/np.mean(s)
    elif keyword_add == 0:
        I_An = 0
        I_Ca = 0
        I_Mg = 0 
    ...
    R_alk[0] = (f_Mg[0]+f_Ca[0]+f_Na[0]+f_K[0])*CEC_tot[0] # [mol_c]
    
    #total amounts (solution and adsorbed)
    Ca_tot[0] = Ca[0]*n*s[0]*Zr*1000+f_Ca[0]/2*CEC_tot[0] # [mol] 
    Mg_tot[0] = Mg[0]*n*s[0]*Zr*1000+f_Mg[0]/2*CEC_tot[0] # [mol] 
    K_tot[0] = K[0]*n*s[0]*Zr*1000+f_K[0]*CEC_tot[0] # [mol]
    Na_tot[0] = Na[0]*n*s[0]*Zr*1000+f_Na[0]*CEC_tot[0] # [mol]
    Alk_tot[0] = 2*Mg_tot[0]+2*Ca_tot[0]+Na_tot[0]+K_tot[0]-An[0]*(n*s[0]*Zr*1000) # [mol_c]
    An_tot[0] = An[0]*n*s[0]*Zr*1000 #[mol_c]
    Al_tot[0] = Al_w[0]*n*Zr*s[0]*1000+(f_Al[0]/3)*CEC_tot[0]*conv_Al # [mol]
    Si_tot[0] = Si[0]*n*Zr*s[0]*1000
    V_w0 = n * Zr * s[0] * 1000                   # [L/m²]
    # Ni_react tracks total reactive Ni [mol/kg], initialised from HNO3 value
    Ni_react = np.zeros(len(s))
    Ni_react[0] = Ni_react_init                   # [mol/kg] 
    Ni_tot[0]   = Ni_react[0] * M_soil * conv_mol # [µmol/m²]
    Ni[0]       = Ni_dis_PTF(Ni_react[0], OM_pct, H[0])  # [µmol/L] PTF 
...
#------------------------------------------------------------------------------
    ...
    for i in range(1, len(s)): 
            ...
            # --- Secondary silica precipitation (CORRECT PLACE) ---
            if keyword_Si_precip == 1:
                Ksp_i = K_sp_SiO2_am[i-1]
                if Si[i-1] > 0 and Ksp_i > 0:
                    SI_SiO2 = np.log10(Si[i-1] / Ksp_i)
                else:
                    SI_SiO2 = -999.0

                if SI_SiO2 > 0:
                    Vw = s[i-1] * n * Zr * 1000
                    R_Si_prec = k_prec_Si * (10**SI_SiO2 - 1) * Si[i-1]  # mol-conv/L/d
                    P_Si[i-1] = R_Si_prec * Vw                            # mol-conv/d
                    Si_sink_other = ((L[i-1]+T[i-1])*1000*Si[i-1] + UP_Si[i-1]) * dt
                    Si_available = max(0.0, Si_tot[i-1] - Si_sink_other)
                    P_Si[i-1] = min(P_Si[i-1], Si_available / dt)
                    P_Si[i-1] = max(0.0, P_Si[i-1])
            ...
            Si_tot[i] = Si_tot[i-1]+(I_Si+np.sum(min_st[:,5]*EW[:,i-1])-(L[i-1]+T[i-1])*1000*Si[i-1]-UP_Si[i-1]-P_Si[i-1])*dt
            ...
            SiO2_gel[i] = SiO2_gel[i-1] + P_Si[i-1] * dt
            Ni_tot[i] = Ni_tot[i-1] + (I_Ni+ np.sum(min_st[:,6] * EW[:,i-1]) - (L[i-1] + T[i-1]) * 1000 * Ni[i-1]) * dt
            ...
            def equations(p):
                ...
                return((Alk_tot[i]-R_alk[i])-Alk[i]*(n*Zr*s[i]*1000),\
                       ...
                       R_alk[i]-(f_Mg[i]+f_Ca[i]+f_Na[i]+f_K[i])*CEC_tot[i],\
                       Al_w[i]*n*Zr*s[i]*1000+(f_Al[i]/3)*CEC_tot[i]*conv_Al-Al_tot[i],\
                       Al[i]-(H[i]**4/(H[i]**4+H[i]**3*K1+H[i]**2*K1*K2+H[i]*K1*K2*K3+K1*K2*K3*K4))*Al_w[i],\
                       Mg[i]*n*Zr*s[i]*1000+f_Mg[i]/2*CEC_tot[i]-Mg_tot[i],\
                       Ca[i]*n*Zr*s[i]*1000+f_Ca[i]/2*CEC_tot[i]-Ca_tot[i],\
                       Na[i]*n*Zr*s[i]*1000+f_Na[i]*CEC_tot[i]-Na_tot[i],\
                       K[i]*n*Zr*s[i]*1000+f_K[i]*CEC_tot[i]-K_tot[i],\
            ...
            R_alk0 = R_alk[i-1]
            Al_w0 = (Al_tot[i]-(f_Al[i-1]/3)*CEC_tot[i]*conv_Al)/(n*Zr*s[i]*1000)#s[i-1]*Al_w[i-1]/s[i]
            Al0 = (H[i-1]**4/(H[i-1]**4+H[i-1]**3*K1+H[i-1]**2*K1*K2+H[i-1]*K1*K2*K3+K1*K2*K3*K4))*Al_w0
            Mg0 = (Mg_tot[i]-f_Mg[i-1]/2*CEC_tot[i])/(n*Zr*s[i]*1000) #s[i-1]*Mg[i-1]/s[i] 
            Na0 = (Na_tot[i]-f_Na[i-1]*CEC_tot[i])/(n*Zr*s[i]*1000) #s[i-1]*Na[i-1]/s[i] 
            Ca0 = (Ca_tot[i]-f_Ca[i-1]/2*CEC_tot[i])/(n*Zr*s[i]*1000) #s[i-1]*Ca[i-1]/s[i]
            K0 =  (K_tot[i]-f_K[i-1]*CEC_tot[i])/(n*Zr*s[i]*1000) #s[i-1]*K[i-1]/s[i]
            H0 = H[i-1]
            def eqH(p):
                    H0 = p
            ...
            if np.any(abs(errors[:,i]) > res_threshold):
                ...
                if np.any(abs(errors[:,i]) > res_threshold):
                    print(i)
                    # ---- DEBUG: print why it fails ----
                    try:
                        max_res = np.max(np.abs(errors[:, i]))
                        idx = int(np.argmax(np.abs(errors[:, i])))
                    except Exception:
                        max_res, idx = None, None

                    print("\n=== fsolve NOT converging ===")
                    print("i =", i, " / t(day) =", i*dt)
                    print("s[i] =", float(s[i]), " s[i-1] =", float(s[i-1]))
                    print("pH[i-1] =", float(pH[i-1]), " H[i-1] =", float(H[i-1]/conv_mol))
                    print("max |res| =", max_res, " at eq index =", idx)
                    print("residual vector =", errors[:, i])
                    print("Alk_tot =", float(Alk_tot[i]), " IC_tot =", float(IC_tot[i]))
                    print("Ca_tot =", float(Ca_tot[i]), " Mg_tot =", float(Mg_tot[i]), " Si_tot =", float(Si_tot[i]))
                    print("Ca =", float(Ca[i-1]), " Mg =", float(Mg[i-1]), " Si =", float(Si[i-1]))
                    print("Omega_CaCO3(prev) =", float(Omega_CaCO3[i-1]), " Omega_MgCO3(prev) =", float(Omega_MgCO3[i-1]))
                    print("================================\n")
                    # ---- DEBUG end ----
            ...
            V_w_i = n * Zr * s[i] * 1000
            Ni_react[i] = max(Ni_tot[i] / (M_soil * conv_mol), 1e-15)  # [mol/kg]
            Ni[i] = Ni_dis_PTF(Ni_react[i], OM_pct, H[i])  # PTF: Groenenberg et al. (2010)
