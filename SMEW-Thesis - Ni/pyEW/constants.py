...
def min_const(mineral, T_K, conv_mol):
    ...
    # All values are from Palandri (2004)
    # The exceptions are the solubility product constants (K_sp), which, when available, are from Morel et al., (1993) -- https://booksite.elsevier.com/9780120885305/appendices/Web_Appendices.pdf
    ...
    if mineral == 'forsterite': #Mg2SiO4
            ...
            n_OH = 1
            min_st = [0, 2, 0, 0, 0, 1, 0.007]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(7.11) 
            ...
            n_H = 0.47 # reaction order
            n_OH = 1
            min_st = [0, 1, 0, 0, 0, 1, 0.007]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan      
            ...
            n_OH = 1
            min_st = [1, 0, 0, 0, 0, 1, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(6.82)
    
    elif mineral == 'fayalite': #Fe2SiO4
            MM_min =  203.77/conv_mol # [g/mol-conv]
            k_diss_H = 10**(-4.80)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_w = 10**(-12.80)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_OH = 0
            E_H = 94.4/conv_mol# [kJ/mol-conv]: activation energies 
            E_w = 94.4/conv_mol# [kJ/mol-conv]
            E_OH = 0
            n_H = 1 # reaction order
            n_OH = 1
            min_st = [0, 0, 0, 0, 0, 1, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(4.21)

    elif mineral == 'lizardite': #Mg3(Si2O5)(OH)4,
            MM_min = 277.1/conv_mol # [g/mol-conv]
            k_diss_H = 10**(-7.8)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_w = 10**(-12.4)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_OH = 0
            E_H = 75/conv_mol# [kJ/mol-conv]: activation energies 
            E_w = 56.6/conv_mol# [kJ/mol-conv]
            E_OH = 0
            n_H = 0.8 # reaction order
            n_OH = 1
            min_st = [0, 3, 0, 0, 0, 2,0.013]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan

    elif mineral == 'enstatite': #MgSiO3
            MM_min = 100.39/conv_mol # [g/mol-conv]
            k_diss_H = 10**(-9.02)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_w = 10**(-12.72)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_OH = 0
            E_H = 80/conv_mol# [kJ/mol-conv]: activation energies 
            E_w = 80/conv_mol# [kJ/mol-conv]
            E_OH = 0
            n_H = 0.6 # reaction order
            n_OH = 1
            min_st = [0, 1, 0, 0, 0, 1, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(5.82)

    elif mineral == 'clinochlore': #(Mg,Fe)5​Al(Si3​Al)O10​(OH)8​
            MM_min = 556/conv_mol # [g/mol-conv]
            k_diss_H = 10**(-11.11)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_w = 10**(-12.52)*24*3600*conv_mol # [mol-conv m-2 d-1]
            k_diss_OH = 0
            E_H = 88/conv_mol# [kJ/mol-conv]: activation energies 
            E_w = 88/conv_mol# [kJ/mol-conv]
            E_OH = 0
            n_H = 0.5 # reaction order
            n_OH = 1
            min_st = [0, 5, 0, 0, 2, 3, 0.028]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan
            ...
            n_OH = - 0.572
            min_st = [0, 0, 0, 1, 1, 3, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(-0.68)
            ...
            n_OH = 1
            min_st = [1, 0, 0, 0, 2, 2, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(9.83)
            ...
            n_H =  0.6 # reaction order
            n_OH = 1
            min_st = [0.6, 0, 0, 0.4, 1.6, 2.4, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan 
            ...
            n_OH = 1
            min_st = [0.9, 0.9, 0, 0.1, 0.4, 1.9, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan  
            ...
            n_H =  0.7 # reaction order
            n_OH = -0.2
            min_st = [0, 0, 1, 0, 1, 2, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan  
            ...
            n_OH = 1
            min_st = [1, 1, 0, 0, 0, 2, 0.00058]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = 10**(5.30)
            ...
            n_H =  0.5 # reaction order
            n_OH = -0.82
            min_st = [0.03, 0, 0.41, 0.56, 1.03, 2.97, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan
            ...
            n_OH = 1
            min_st = [5, 0, 0, 0, 0, 0, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan
            ...
            n_OH = -0.2
            min_st = [0, 0, 0.25, 0.75, 1, 1, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
            K_sp = np.nan
            ...
            n_OH = -0.22
            min_st = [0, 0, 1, 0, 3, 3, 0]# Stochiometric coefficients [Ca, Mg, K, Na, Al, Si, Ni]
...
def MM(conv_mol):
    ...
    return(MM_Mg, MM_Ca, MM_Na, MM_K, MM_Si, MM_C, MM_Anions, MM_Al, MM_Ni)

def K_sp_SiO2_am(T_K, conv_mol):
    """
    Solubility of amorphous SiO2
    log Ksp = -8.476 - 485.24/T - 2.268e-6*T^2 + 3.068*log(T)
    Gunnarsson & Arnorsson (2000), GCA 64, 2295-2307
    At 25C: Ksp ~ 1.95e-3 mol/L ~ 1950 umol/L
    """
    log_Ksp = -8.476 - 485.24/T_K - 2.268e-6 * T_K**2 + 3.068 * np.log10(T_K)
    K_sp = 10**log_Ksp * conv_mol  # [mol-conv/L = µmol/L]
    return K_sp