...
result = {grp: {'v': np.array([])} for grp in Group}

for idx, (start_s, end_s, df_season) in enumerate(seasons):
    # add gap of zero growth between seasons if needed
    if idx > 0:
        prev_end = pd.Timestamp(seasons[idx-1][1])
        curr_start = pd.Timestamp(start_s)
        gap_days = (curr_start - prev_end).days - 1
        if gap_days > 0:
            for grp in Group:
                result[grp]['v'] = np.concatenate([result[grp]['v'], np.zeros(int(gap_days/dt))])
    ...
    for _, row in df_season.iterrows():
        ...
        result[treatment]['v'] = np.concatenate([result[treatment]['v'], v_full[:n_steps]])
