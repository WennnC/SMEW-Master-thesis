...
def Omega_sil(mineral, Ca, Mg, K, Na, Si, H, K_sp, conv_mol):
        
    if mineral == 'forsterite':
        Omega = (Mg/conv_mol)**(1/2)*(Si/conv_mol)**(1/4)/(H/conv_mol)/K_sp
    
    elif mineral == 'wollastonite':
        Omega = (Ca/conv_mol)**(1/2)*(Si/conv_mol)**(1/2)/(H/conv_mol)/K_sp
    
    elif mineral == 'diopside':
        #CaMgSi2O6 + (H+) -> 1/4 Ca++ + 1/4Mg++ 1/2 H2SiO3
        Omega = (Ca/conv_mol)**(1/4)*(Mg/conv_mol)**(1/4)*(Si/conv_mol)**(1/2)/(H/conv_mol)/K_sp
    
    elif mineral == 'albite':
        #NaAlSiO3 + (H+) + 5/2 H2O -> 1/2 kaolinite + 2HSiO3 + (Na+)
        Omega = (Na/conv_mol)*(Si/conv_mol)**2/(H/conv_mol)/K_sp    
    
    elif mineral == 'anorthite':
        #1/2 CaAl2Si2O8+ (H+) + 1/2 H2O -> 1/2 kaolinite + 1/2 (Ca++)
        Omega = (Ca/conv_mol)**(1/2)/(H/conv_mol)/K_sp
        
    elif mineral == 'muscovite':
        #KAl3Si3O10(OH)2 + (H+) + 3/2 H2O -> 3/2 kaolinite + K+
        Omega = (K/conv_mol)/(H/conv_mol)/K_sp

    elif mineral in ['labradorite', 'augite', 'alkali_feldspar', 'Fe_forsterite', 'nepheline','apatite','leucite','fayalite','clinochlore','lizardite']:
        Omega = 0
    
    elif mineral == 'enstatite':
        #1/2 Mg2Si2O6 + (H+) +1/2 H2O -> 1/2 Mg++ + 1/2 H4SiO4
        Omega = (Mg/conv_mol)**(1/2)*(Si/conv_mol)**(1/2)/(H/conv_mol)/K_sp
