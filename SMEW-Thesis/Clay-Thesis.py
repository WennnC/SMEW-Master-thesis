...
f_Na_in = 0.007
f_Al_in =  0.005
Si_in = 1093.83*1e-9*rho_bulk/MM_Si #[micromol/l]
#base saturation
...
CaCO3_in = 0.259 * (100.09/12.01) * 1.54e6 * 0.2 / 100.09 * 1e6 / 1e3
MgCO3_in = 0
...
for name, path in path_dict.items():
    ...
    df = df[df['Sampling date'].astype(str).str.contains('2022') & df['Treatment'].astype(str).str.contains('Clay', case=False, na=False)]
    if name == 'CaCl2':
        df = df[cols_cacl2]
    else:
...
#select row
df_pH_25 = df_pH.iloc[27:33]
processed_data['pH_25cm'] = df_pH_25
df_pH_12 = df_pH.iloc[8:14]
...
# CaCl2 Si（Sept_2022）
si_vals  = processed_data['Si_CaCl2'].get('Olivine - Clay', [])
...
for trt in ['Wollastonite - Clay', 'Wollastonite - Biochar - Clay']:
    obs = df_BaCl2_sept.loc[trt, 'Ca (mg/kg)']
    diss_f_dict[trt] = calibrate_diss_f(trt, obs, 'Ca')

# Control （diss_f=0）
diss_f_dict['Control - Clay'] = 0
diss_f_dict['Control - Biochar - Clay'] = 0
...
data_OC  = biogeochem_precip['Olivine - Clay']
data_OBC = biogeochem_precip['Olivine - Biochar - Clay']
data_WC  = biogeochem_precip['Wollastonite - Clay']
data_WBC = biogeochem_precip['Wollastonite - Biochar - Clay']
data_CC  = biogeochem_precip['Control - Clay']
data_CBC = biogeochem_precip['Control - Biochar - Clay']
...
for i in Group:
    ...
    for ion in ions: 
        ...
        for method in ['HNO3', 'CaCl2', 'BaCl2']: 
            ...
            if 'Start soil clay' in df.index:
                val_start = df.loc['Start soil clay', col_name]
...
legend_biochar = [
    mlines.Line2D([], [], color='gray', linestyle='--', linewidth=2.5, label='Model(Without Biochar)'),
    mlines.Line2D([], [], color='gray', linestyle='-',  linewidth=2.5, label='Model(With Biochar)')
]
axes[1].legend(handles=legend_lines + legend_markers + legend_biochar, 
               ...
               frameon=False, 
               fontsize=13)         
fig2.subplots_adjust(wspace=0.2, left=0.07, right=0.97, top=0.92, bottom=0.2)
plt.show()
    ...
    for method in ['HNO3','BaCl2','CaCl2']:
        ...
        for i, g in enumerate(Group):
            ...
            if g not in df.index:
                continue
            if 'Start soil clay' not in df.index:
                continue

            start = df.loc['Start soil clay', col_name]
...
#%% Fraction of CEC
# Fraction of CEC
f_Ca_list = [data_CBC['f_Ca'], data_OBC['f_Ca'], data_WBC['f_Ca']]
f_Mg_list = [data_CBC['f_Mg'], data_OBC['f_Mg'], data_WBC['f_Mg']]
f_K_list = [data_CBC['f_K'], data_OBC['f_K'], data_WBC['f_K']]
f_Na_list = [data_CBC['f_Na'], data_OBC['f_Na'], data_WBC['f_Na']]
f_Al_list = [data_CBC['f_Al'], data_OBC['f_Al'], data_WBC['f_Al']]
f_H_list = [data_CBC['f_H'], data_OBC['f_H'], data_WBC['f_H']]
titles = ['Control - Biochar - Clay', 'Dunite - Biochar - Clay', 'Wollastonite - Biochar - Clay']
figCEC = pyEW.fig_CEC(t, f_Ca_list, f_Mg_list, f_K_list, f_Na_list, f_Al_list, f_H_list, titles)

f_Ca_list = [data_CC['f_Ca'],data_OC['f_Ca'],data_WC['f_Ca']]
f_Mg_list = [data_CC['f_Mg'],data_OC['f_Mg'],data_WC['f_Mg']]
f_K_list = [data_CC['f_K'],data_OC['f_K'],data_WC['f_K']]
f_Na_list = [data_CC['f_Na'],data_OC['f_Na'],data_WC['f_Na']]
f_Al_list = [data_CC['f_Al'],data_OC['f_Al'],data_WC['f_Al']]
f_H_list = [data_CC['f_H'],data_OC['f_H'],data_WC['f_H']]
titles = ['Control - Clay', 'Dunite - Clay', 'Wollastonite - Clay']
figCEC = pyEW.fig_CEC(t, f_Ca_list, f_Mg_list, f_K_list, f_Na_list, f_Al_list, f_H_list, titles)
...
# --- Method 3: IC method ---
def IC_method(data_IC, data_DIC, data_CaCO3, data_MgCO3, treatment_name):
    delta_DIC_L  = np.trapezoid(data_DIC * 1000 * L, t)
    delta_IC_s   = (data_IC[-1]   - data_IC[0])
    delta_CaCO3  = (data_CaCO3[-1] - data_CaCO3[0])
    delta_MgCO3  = (data_MgCO3[-1] - data_MgCO3[0])
    gCO2 = 44 * (delta_DIC_L + delta_IC_s + delta_CaCO3 + delta_MgCO3) / (conv_mol * rho_bulk * 1e-3 * Zr)
    return gCO2
...
# Control baselines
Mg_CS  = data_CC['Mg_tot'][-1]  / conv_mol + np.trapezoid(data_CC['UP_Mg'],  t) / conv_mol
Ca_CS  = data_CC['Ca_tot'][-1]  / conv_mol + np.trapezoid(data_CC['UP_Ca'],  t) / conv_mol
Mg_CBS = data_CBC['Mg_tot'][-1] / conv_mol + np.trapezoid(data_CBC['UP_Mg'], t) / conv_mol
Ca_CBS = data_CBC['Ca_tot'][-1] / conv_mol + np.trapezoid(data_CBC['UP_Ca'], t) / conv_mol

for i in Group:
    if 'Olivine' in i:
        base_cation_conc = {
            'Ca': 3005*1e-6/40, 'Mg': 231670*1e-6/24.3,
            'K': 520*1e-6/39.1, 'Na': 483*1e-6/23
        }
        CDR_pot = 0.87
        data_current = data_OBC if 'Biochar' in i else data_OC  # ✅ 用 Clay 变量名
        ...
        Mg_control      = Mg_CBS if 'Biochar' in i else Mg_CS
        M_rock_in       = 29 * rho_bulk * Zr / 1000
        Mg_mineral      = M_rock_in * base_cation_conc['Mg']
        Fw, CDR_M1      = Method1_EMB(Mg_treatment, Mg_control, Mg_mineral, M_rock_in, CDR_pot)
        Mg_leachate     = np.trapezoid(result[i]['Mg_leachate'] / 1e3 * A_container, t)
        leached_fraction = min(Mg_leachate / (Fw * Mg_mineral), 1.0) if (Fw > 0 and Mg_mineral > 0) else 0
        Leached_CDR_M1  = CDR_M1 * leached_fraction

    elif 'Wollastonite' in i:
        base_cation_conc = {
            'Ca': 171026*1e-6/40, 'Mg': 22312*1e-6/24.3,
            'K': 18844*1e-6/39.1, 'Na': 15431*1e-6/23
        }
        CDR_pot = 0.76
        data_current = data_WBC if 'Biochar' in i else data_WC  # ✅ 用 Clay 变量名
        ...
        Ca_control      = Ca_CBS if 'Biochar' in i else Ca_CS
        M_rock_in       = 29 * rho_bulk * Zr / 1000
        Ca_mineral      = M_rock_in * base_cation_conc['Ca']
        Fw, CDR_M1      = Method1_EMB(Ca_treatment, Ca_control, Ca_mineral, M_rock_in, CDR_pot)
        Ca_leachate     = np.trapezoid(result[i]['Ca_leachate'] / 1e3 * A_container, t)
        leached_fraction = min(Ca_leachate / (Fw * Ca_mineral), 1.0) if (Fw > 0 and Ca_mineral > 0) else 0
        Leached_CDR_M1  = CDR_M1 * leached_fraction
    ...
    result[i].update({'CDR': CDR_M1, 'Leached_CDR': Leached_CDR_M1, 'Fw': Fw})

# Method 3
CO2_OBS_M3 = IC_method(data_OBC['IC_tot'], data_OBC['DIC'], data_OBC['CaCO3'], data_OBC['MgCO3'], 'Olivine - Biochar - Clay')
CO2_OS_M3  = IC_method(data_OC['IC_tot'],  data_OC['DIC'],  data_OC['CaCO3'],  data_OC['MgCO3'],  'Olivine - Clay')
CO2_WBS_M3 = IC_method(data_WBC['IC_tot'], data_WBC['DIC'], data_WBC['CaCO3'], data_WBC['MgCO3'], 'Wollastonite - Biochar - Clay')
CO2_WS_M3  = IC_method(data_WC['IC_tot'],  data_WC['DIC'],  data_WC['CaCO3'],  data_WC['MgCO3'],  'Wollastonite - Clay')
CO2_CBS_M3 = 0.0
CO2_CS_M3  = 0.0

# Observations: EMB method — 
Fw_obs_dict = {
    'Olivine - Clay':                {'Fw': 0.254, 'CDR_pot': 0.87},  
    'Olivine - Biochar - Clay':      {'Fw': 0.230, 'CDR_pot': 0.87},
    'Wollastonite - Clay':           {'Fw': 0.379, 'CDR_pot': 0.76},
    'Wollastonite - Biochar - Clay': {'Fw': 0.12, 'CDR_pot': 0.76},
}
M_rock_in = 26 * rho_bulk * Zr / 1000
...
ax.bar(x + width/2, CDR_obs_EMB, width, label='Observed - EMB',
       color='#4C72B0', alpha=0.4, edgecolor='black', linewidth=0.6, hatch='//')
...
for i in Group:
    ...
    for ion in ions:
        ...
        for method in ['HNO3', 'CaCl2', 'BaCl2']:
            ...
            if 'Start soil clay' in df.index:
                ax.scatter(0, df.loc['Start soil clay', col_name],
...
si_color_base   = '#4878CF'
si_color_precip = '#D65F5F'
si_color_obs_c  = '#6ACC65'
si_color_obs_h  = '#B47CC7'

annotated_axes = set()   # track annotated axes to avoid duplicate labels
...
for i in Group:
    ...
    # Base model Si (no Si precipitation)
...
for ax, title in zip(axes_si, titles_si):
    ...
    ax.grid(True, linestyle='--', alpha=0.4)
    ax.set_ylim(bottom=0)
axes_si[0].set_ylabel("Si in soil (mg/kg)", fontsize=14)
...
_calib_data = {
    'Olivine - Clay':                {'diss_f': 0.4015,  'k_Si': 0.14722,
                                      'diss_f_err': 0.007, 'k_Si_err': 0.069},
    'Olivine - Biochar - Clay':      {'diss_f': 0.3897,  'k_Si': 0.14708,
                                      'diss_f_err': 0.041, 'k_Si_err': 0.011},
    'Wollastonite - Clay':           {'diss_f': 0.9353,  'k_Si': 0.14708,
                                      'diss_f_err': 0.213, 'k_Si_err': 0.088},
    'Wollastonite - Biochar - Clay': {'diss_f': 0.9353,  'k_Si': 0.14708,
                                      'diss_f_err': 0.230, 'k_Si_err': 0.031},
    'Control - Clay':                {'diss_f': 0.0,     'k_Si': 0.05103,
                                      'diss_f_err': 0.0,  'k_Si_err': 0.629},
    'Control - Biochar - Clay':      {'diss_f': 0.0,     'k_Si': 0.05103,
...
# open = Clay only;  filled = Biochar + Clay
...
# Clay experiment: start 2022-05-23, end 2022-09-19 (119 days)
start_date_val = datetime.date(2022, 5, 23)
date_to_simday = {
    'May_2022':  0,
    'Sept_2022': (datetime.date(2022, 9, 19) - start_date_val).days,  # 119
}

def get_treatment_clay(sample_name):
...
for i in Group:
    ...
    for sample_name in df_pH_val.index:
        if get_treatment_clay(sample_name) != i:
            continue
...
# Clay only has May_2022 and Sept_2022 data
# ─────────────────────────────────────────────────────────────────────
ion_cols = ['Ca (mg/kg)', 'Mg (mg/kg)', 'K (mg/kg)', 'Na (mg/kg)']
df_bacl2_clay = real_data_dict['BaCl2'].reset_index()

for i in Group:
    df_trt = df_bacl2_clay[df_bacl2_clay['Treatment'] == i]
    if df_trt.empty:
        continue
    for ion in ions:
        col_name = f'{ion} (mg/kg)'
        if col_name not in df_trt.columns:
            continue
        obs_ion, t_obs_ion = [], []
...
print('\n' + '='*80)
print('Model Validation — NRMSE (Base & Precip vs Observations) — Clay')
print('='*80)
